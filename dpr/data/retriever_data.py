...
TableChunk = collections.namedtuple(
    "TableChunk", 
    ["text", "title", "table_id"]
)
...
class RetrieverData(torch.utils.data.Dataset):
    def __init__(self, file: str):
        """
        :param file: 
        - real file name or the resource name as they 
          are defined in download_data.py
        """
        self.file = file
        self.data_files = []
    ...
    def load_data(self):
        ...
        ), ("RetrieverData source currently works with single files only. "
            "Files specified: {}".format(
            self.data_files
        ))
...
class QASrc(RetrieverData):
    def __init__(
        ...
        self.data = None
        self.selector = (
            hydra.utils.instantiate(selector) if selector else None
        )
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        ...
        with open(self.file) as ifile:
            ...
            for i, row in enumerate(reader):
                ...
                # print(f"\"{sample_id}\" - {type(sample_id)}")
